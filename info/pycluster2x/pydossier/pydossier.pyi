"""
    ...
    各个模态中管理的每个档案，对应一个该模态的档案id（mid），每个模态的档案id是连续的[0,cluster_num)。
    ...
    档案集中管理的逻辑人在档案集中的id称为人的id（person_id,pid），人的id是连续的,[0,person_num)。指向同一个pid的不同模态的不同档案相互关联，指向不同pid的档案则不是同一个人的档案，不关联。
...
import typing
__all__ = ['async_load', 'async_load_big', 'async_load_small', 'async_unser', 'dossier', 'fuse_e', 'load', 'load_big', 'load_small', 'oc_updates', 'unser']
class dossier:
    """
    档案集类
    """
...
def load(path: str) -> dossier:
    """
            ...
            dossier.save("./record/")
            dossier_0:pydossier.dossier = pydossier.load("./record/")
            ```
            support asynchronous
    """
@typing.overload
def load(path: str, cfg: dict[str, str]) -> dossier:
    """
            读取档案，对应通过save保存的档案格式，同时重新指定配置参数
            例如：
            ``` python
...
def load_big(path: str, cfg: dict[str, str]) -> dossier:
    """
            读取档案，对应通过save_big保存的档案格式，同时重新指定配置参数
            例如：
            ``` python
...
def load_small(path: str) -> dossier:
    """
            ...
            dossier.save_small("./record/")
            dossier_0:pydossier.dossier = pydossier.load_small("./record/")
            ```
            support asynchronous
    """
@typing.overload
def load_small(path: str, cfg: dict[str, str]) -> dossier:
    """
            读取档案，对应通过save_small保存的档案格式，同时重新指定配置参数
            例如：
            ``` python
